      ...
      templates: {
        item: function (data) {
          ...
          if (data.categories && Array.isArray(data.categories)) {
            var catNames = data.categories.map(function (cat) { return cat.name || cat; });
            cats = '<span>' + catNames.join('<i class="ic i-angle-right"></i>') + '</span>';
          }
          var title = data._highlightResult && data._highlightResult.title && data._highlightResult.title.value
            ? data._highlightResult.title.value
            : data.title || '';
          var url = data.permalink || data.path || '';
          if (url && !url.startsWith('http')) {
            url = CONFIG.root + url;
          }
          return '<a href="' + url + '">' + cats + title + '</a>';
        },
        empty: function (data) {
